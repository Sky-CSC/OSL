...
namespace OSL_CDragon
{
    ...
    {
        private static Logger _logger = new("PerksManager");
        ...
        public static void DownloadPerks(string numPatch, string region)
        {
            ...
            string perksDirectory = CDragon.perksDirectory;
        ...
        public static void DownloadPerksComposent(string numPatch, string region, string perksDirectory, int indexPatch, int indexRegion)
        {
            ...
            string perksData = OSL_Common.Download.Download.DownloadStringAsync(urlPerks).Result;
            if (perksData != null)
            {
                try
                {
                    ...
                    foreach (dynamic perk in jsonPerks)
                    {
                        try
                        {
                            perksDirectory = CDragon.perksDirectory;
                            int perkId = perk.id;
                    ...
                    {
                        _logger.log(LoggingLevel.INFO, "PerksAsyncDownload()", $"Waiting end DownloadFileAsync() download : {OSL_Common.Download.Download.downloadAllFile} error : {OSL_Common.Download.Download.errorDownloadAllFile}");
                        infini++;
                        Thread.Sleep(100);
                    }
                    _logger.log(LoggingLevel.INFO, "PerksAsyncDownload()", $"{OSL_Common.Download.Download.errorDownloadAllFile} error of download");
        ...
        public static void PerksAsyncDownload(int indexPatch, int indexRegion, string numPatch, dynamic perk, string perksDirectory, int perkId, string perkFullName)
        {
            ...
            OSL_Common.Download.Download.DownloadFileAsync(urlPerkId, perkIcone);
