...
using OSL_Web.Configuration.Overlay.ChampSelect;
...
namespace OSL_Web.Pages.ChampSelect
{
    ...
    {
        private static Logger _logger = new("ChampSelectView1Page");
        ...
        {
            Configuration.Overlay.ChampSelect.View1.Config.LoadFormatingDataConfig(); 
        }
        ...
        private static int SideInProgress()
        {
            foreach (var action in DataProcessing.ChampSelect.session.Actions)
            {
                foreach (var inaction in action)
                {
                    ...
                    {
                        foreach (var inmyTeam in DataProcessing.ChampSelect.session.MyTeam)
                        {
